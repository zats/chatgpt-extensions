...
test("unavailable home transforms reject direct registration", () => {
  resetAdapterState();
  let context: RendererExtensionContext | undefined;
  ...
  const identity = {
    id: "unavailable-home-transform-test",
    version: "1.0.0",
    manifestDigest: "digest",
  };
  const module = {
  ...
  try {
    ...
    for (const point of [
      ...
      assert.throws(
        () => context!.api.contributions.transform(point, (items) => items),
        (error: unknown) => {
          assert.equal(
          ...
          assert.deepEqual(
            (error as { readonly details?: unknown }).details,
            {
              operation: `Contribution transform ${point}`,
              reason: "binding-unavailable",
            },
          );
          return true;
    ...
    assert.equal(legacyRegistrations, 0);
  } finally {
    deactivateExactBuildRendererExtension(identity.id, module, "renderer");
    resetAdapterState();
  }
...
test("the exact-build UI bridge maps sidebar, mode, and composer actions", async () => {
  resetAdapterState();
  interface UiContext {
...
test("extension settings events cross renderer documents", async () => {
  const stateKey = Symbol.for("chatgptx.v5.exact-build-adapter.26.820.80927");
...
test("discovery and runtime capabilities report the complete stable UI catalog", async () => {
  ...
  try {
    ...
    for (const pointId of [
      ...
      const point = transforms.find((candidate) => candidate.id === pointId);
      assert.equal(point?.state, "unavailable");
      assert.equal(point?.unavailableReason, "binding-unavailable");
    }
    assert.deepEqual(
    ...
    for (const capabilityId of [
      "ui.point.home.new-chat-suggestions",
      "ui.point.home.announcements",
    ]) {
      const homeCapability = await capabilities.get(capabilityId);
      assert.equal(homeCapability.state, "unavailable");
      assert.equal(homeCapability.unavailableReason, "binding-unavailable");
      assert.deepEqual(homeCapability.operations, []);
    }
