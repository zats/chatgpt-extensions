...
(() => {
  ...
  // runtime/bindings/26.825.32147/renderer-adapter.ts
  var globalStateKey = Symbol.for("chatgptx.v5.exact-build-adapter.26.825.32147");
  ...
  function homeSuggestionContext(context) {
    return Object.freeze({
      ownerId: context.ownerId,
      windowId: adapterState().windowId,
      composer: legacyComposerState(context),
      composerMode: context.composerMode ?? (context.mode === "work" ? "work" : "chat"),
  ...
  function homeAnnouncementContext(context) {
    return Object.freeze({
      ownerId: context.ownerId,
      windowId: adapterState().windowId,
      composer: legacyComposerState(context),
      entryPoint: context.entryPoint ?? "home",
      homeComposerMode: context.homeComposerMode ?? context.composerMode ?? (context.mode === "work" ? "work" : "chat"),
  ...
  function publicUiAction(item, context) {
    return Object.freeze({
      kind: "action",
      id: item.id,
  ...
  function publicHomeAnnouncementAction(item, context) {
  ...
  function legacyUiAction(item, context, extensionId) {
    return {
      kind: "action",
  ...
  function publicHomeSuggestionItem(item, context, mapping) {
    const appOwned = isAppOwned(item.origin);
    ...
    return Object.freeze({
      ...
      ...item.tooltip === void 0 ? {} : { tooltip: item.tooltip },
      ...item.icon === void 0 ? {} : { icon: item.icon },
      ...item.listIcon === void 0 ? {} : { listIcon: item.listIcon },
      ...item.keyboardShortcut === void 0 ? {} : { keyboardShortcut: item.keyboardShortcut },
      ...item.disabled === void 0 ? {} : { disabled: item.disabled },
      onActivate: (_owner, next) => item.onClick?.(legacyActivation(next)),
      ...item.onDismiss === void 0 ? {} : {
        onDismiss: (_owner, next) => item.onDismiss?.(legacyActivation(next))
      },
      origin: appOwned ? "app" : `extension:${item.origin}`
    });
  }
  function legacyHomeSuggestionItem(item, context, extensionId, mapping) {
    const existing = mapping.rawByPublicId.get(item.id);
    if (item.kind === "opaque") {
      if (!existing) {
    ...
    return {
      ...
      onClick: (next) => item.onActivate(context, activation(next)),
      ...item.onDismiss === void 0 ? {} : {
        onDismiss: (next) => item.onDismiss?.(context, activation(next))
      },
      origin: existing?.origin ?? extensionId
    };
  }
  function publicAnnouncementItem(item, context, mapping) {
    ...
    return Object.freeze({
      ...
      ...item.isLoading === void 0 ? {} : { isLoading: item.isLoading },
      title: item.title,
      ...item.description === void 0 ? {} : { description: item.description },
      ...item.leadingVisual === void 0 ? {} : { leadingVisual: item.leadingVisual },
      ...item.primaryAction === void 0 ? {} : { primaryAction: publicHomeAnnouncementAction(item.primaryAction, context) },
      ...item.dismissAction === void 0 ? {} : { dismissAction: publicHomeAnnouncementAction(item.dismissAction, context) },
      origin: appOwned ? "app" : `extension:${item.origin}`
    });
  }
  function legacyAnnouncementItem(item, context, extensionId, mapping) {
    const existing = mapping.rawByPublicId.get(item.id);
    if (item.kind === "opaque") {
      if (!existing) {
...
`;
          ...
          const register = () => legacy.ui.transformSuggestions((items, legacyContext) => {
            ...
            try {
              ...
              const mapped = items.map(
                (item) => publicHomeSuggestionItem(item, context, mapping)
              );
        ...
        if (point === "home.announcements") {
          ...
          const register = () => legacy.ui.transformAnnouncements((items, legacyContext) => {
            ...
            try {
              ...
              const mapped = items.map(
                (item) => publicAnnouncementItem(item, context, mapping)
              );
              const transformed = transformer(mapped, context, {
  ...
  // runtime/bindings/26.825.32147/renderer-entry.ts
